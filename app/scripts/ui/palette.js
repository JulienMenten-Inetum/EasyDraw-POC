...
function Palette(options, ui) {
  ...
  this.hideOnClick =
    options.hideOnClick === undefined ? true : options.hideOnClick;
  ...
  if (options.split2) {
    this.$element
      .css("display", "grid")
      .css("grid-template-columns", "repeat(10, 1fr)");
  }
  ...
  this.leftOffset = options.hasOwnProperty("leftOffset")
    ? options.leftOffset
    : 5;
  this._closeOnClick = function (e) {
    if (
      !this.hideOnClick &&
      (this.$element === e.target || this.$element.find(e.target).length > 0)
    ) {
      return;
    }
...
Palette.prototype._position = function () {
  ...
  this.$element.css({
    ...
    left:
      p.left +
      anchorButton.$element.outerWidth() -
      mainP.left +
      this.leftOffset,
  });
};
