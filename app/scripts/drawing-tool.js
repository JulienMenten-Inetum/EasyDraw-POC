...
var Eraser = require("./tools/shape-tools/eraser");
var PolygonDrawTool = require("./tools/shape-tools/polygon-draw");
...
var PanTool = require("./tools/pan-tool");
const TopToolBar = require("./ui/top-tools");
const PipetteTool = require("./tools/pipette-tool");
const StampsService = require("./services/stamps");
...
var ADDITIONAL_PROPS_TO_SERIALIZE = [
  "lockUniScaling",
  "objectCaching",
  "isDimension",
  "isDimensionText",
...
function DrawingTool(selector, options, settings) {
  ...
  // Turn on for clip path
  var rect = new fabric.Rect({
    left: 0,
    top: 0,
    fill: "#ffffff",
    width: this.canvas.width + 18,
    height: this.canvas.height + 58,
    stroke: "#9a86e1",
    strokeWidth: 1,
    strokeDashArray: [5, 5],
    selectable: false,
...
DrawingTool.prototype.ADDITIONAL_PROPS_TO_SERIALIZE =
  ADDITIONAL_PROPS_TO_SERIALIZE;

DrawingTool.prototype.setDimensionText = function (text) {
  //fabric.log(text)
  ...
  } else if (selection.isDimensionText) {
    //fabric.log(selection);
    selection.text = text;
    this.canvas.renderAll();
  }
};

DrawingTool.prototype.startPan = function (event, canv) {
  if (event.button != 2) {
    return;
  }
  ...
  function continuePan(event) {
    var x = event.screenX,
      y = event.screenY;
    canv.relativePan({ x: x - x0, y: y - y0 });
    x0 = x;
    y0 = y;
  }
...
DrawingTool.prototype.svg = function (image) {};
...
DrawingTool.prototype.save = function () {
  ...
  //fabric.log(canvasJSON)
  if (!!canvasJSON.objects && canvasJSON.objects.length > 0) {
    canvasJSON.objects = canvasJSON.objects.filter((o) => !o.isDimensionText);
  }
 ...
 */
DrawingTool.prototype.load = function (
  jsonOrObject,
  callback,
  noHistoryUpdate
) {
  ...
  if (fabric.Annotations) {
    // *[ Disabled to enable resize after load
...
DrawingTool.prototype.redo = function () {
  ...
  // fabric.log(this.canvas.toJSON(ADDITIONAL_PROPS_TO_SERIALIZE))
};
...
DrawingTool.prototype.setSelectionStrokeColor = function (color) {
  ...
  this.forEachSelectedObject(
    function (obj) {
      if (obj.type === "group") {
        this.handleGroupSettings(obj, "stroke", color);
      }
...
DrawingTool.prototype.handleGroupSettings = function (obj, parameter, color) {
  obj.getObjects().forEach(
    function (subObj) {
      if (subObj.type === "group") {
        this.handleGroupSettings(subObj, parameter, color);
      }
      this._setObjectProp(subObj, parameter, color);
    }.bind(this)
  );
};
...
DrawingTool.prototype.setSelectionFillColor = function (color) {
  ...
  this.forEachSelectedObject(
    function (obj) {
      ...
      if (obj.type === "group") {
        this.handleGroupSettings(obj, "fill", color);
      }
    }.bind(this)
  );
  this.canvas.renderAll();
...
DrawingTool.prototype.setSelectionStrokeWidth = function (width) {
  ...
  this.forEachSelectedObject(
    function (obj) {
      ...
      if (obj.type === "group") {
        this.handleGroupSettings(obj, "strokeWidth", width);
      }
    }.bind(this)
  );
  this.canvas.renderAll();
...
DrawingTool.prototype.setSelectionStrokeType = function (type) {
  if (!this.getSelection()) return;
  this.forEachSelectedObject(
    function (obj) {
      this._setObjectProp(obj, "strokeDashArray", [type]);
    }.bind(this)
  );
  this.canvas.renderAll();
  this.pushToHistory();
...
DrawingTool.prototype.fillGradient = function (from, to) {
  if (!this.getSelection()) return;
  this.canvas.getActiveObject().setGradient("fill", {
    x1: this.canvas.getActiveObject().width / 2,
    y1: 0,
    x2: this.canvas.getActiveObject().width / 2,
...
DrawingTool.prototype.fillPattern = function (img_location) {
  //fabric.log(img_location)
  if (!this.getSelection()) return;
  this.canvas.getActiveObject().setPatternFill(
    {
      source: img_location,
      repeat: "repeat",
    },
...
DrawingTool.prototype.addToGroup = function (objects = null) {
  if (!this.getSelection()) return;
  var selection = null;
  if (objects === null) {
  ...
  var active = {
    ...
    top: this.canvas.getActiveObject().top,
    width: this.canvas.getActiveObject().width,
    height: this.canvas.getActiveObject().height,
  };
  ...
  this.forEachSelectedObject(
    function (obj) {
      //obj.top = obj.top -this.canvas.getActiveObject().top/2 + obj.height/2
      selection_objects.push(obj);
      // group.addWithUpdate(obj.clone());
      this.canvas.remove(obj);
    }.bind(this)
  );
  //fabric.log(group.getObjects())
  var items = group.getObjects();
  // TODO MOVE OBJECT

  group.setCoords();
  group.lockUniScaling = false;
  // group.left = active.left
  // group.top = active.right

  this.canvas.add(group);
  this.select(group);
  this.canvas.getActiveObject().left = active.left;
  this.canvas.getActiveObject().top = active.top;
  this.canvas.renderAll();
  var tmp_state = this.save();
  this.clear();
  this.load(tmp_state);
};

DrawingTool.prototype.removeFromGroup = function () {
  //fabric.log(this._history)
  if (!this.getSelection()) return;
  var group = this.getSelection();
  this.getSelection().forEachObject((i) => {
    this.getSelection().removeWithUpdate(i);
    this.canvas.add(i);
  });
...
DrawingTool.prototype.lockSelection = function () {
  if (!this.getSelection()) return;
  // fabric.log(this.getSelection().selectable);
  this.forEachSelectedObject(function (obj) {
    obj.set({ selectable: false, lock_selection: true });
  });
  // this.getSelection().forEach(e =>
  //     e.set({selectable:false, lock_selection:true})
  // );
  // fabric.log(this.getSelection().selectable);
  this.clearSelection();
  this.pushToHistory();
...
DrawingTool.prototype.unlockLockSelections = function () {
  var items = this.canvas.getObjects();
  items.forEach((e) => {
    if (!e.keep_unselected) {
      e.set({
        selectable: true,
 ...
 */
DrawingTool.prototype.setBackgroundImage = function (
  imageSrcOrOptions,
  fit,
  callback
) {
  var imageSrc =
    typeof imageSrcOrOptions === "string"
      ? imageSrcOrOptions
      : imageSrcOrOptions.src;
  var imageOptions =
    typeof imageSrcOrOptions === "object" ? imageSrcOrOptions : null;
 ...
 */
DrawingTool.prototype._setBackgroundImage = function (
  imageSrc,
  _options,
  ...
  function callback(img) {
    ...
    if (
      (options.crossOrigin === "anonymous" || options.crossOrigin === "") &&
      !img
    ) {
...
DrawingTool.prototype._initTools = function () {
  ...
  this.tools = {
    ...
    pan: new PanTool("Pan Tool", this),
    arrow: new LineTool("Arrow Tool", this, "arrow"),
    doubleArrow: new LineTool("Double Arrow Tool", this, "arrow", {
      doubleArrowhead: true,
    }),
    dimension: new LineTool("Dimension Tool", this, "arrow", {
      isDimension: true,
      doubleArrowhead: true,
      dim: "100",
    }),
    ...
    polygon: new PolygonDrawTool("Polygon Tool", this),
    ...
    fillPipette: new PipetteTool("Pipette tool", this, "fill"),
    strokePipette: new PipetteTool("Pipette tool", this, "stroke"),
  };
};
...
DrawingTool.prototype._initDOM = function () {
  ...
  this.$canvas = $("<canvas>").appendTo($canvasContainer);
};
...
DrawingTool.prototype._initFabricJS = function () {
  ...
  this.canvas._isPerPixelTargetFindAllowed =
    !fabric.isTouchSupported && this.options.parseSVG;
...
DrawingTool.prototype._setDimensions = function (width, height) {
  ...
  //         .css("width", width)
  //         .css("height", height);
  //     canvEl.getContext("2d", { willReadFrequently: true }).scale(pixelRatio, pixelRatio);
  // }
};
...
DrawingTool.prototype.unGroup = function () {
...
DrawingTool.prototype.loadImage = function (img) {
  ...
  // let image = new fabric.Image.fromURL(img)
  this.canvas.add(image);
  this.canvas.renderAll();
};

DrawingTool.prototype.setStamps = function (stamps) {
  this.options.stamps = stamps;
};
