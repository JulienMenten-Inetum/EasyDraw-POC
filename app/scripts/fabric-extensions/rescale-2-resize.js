...
function basicWidthHeightTransform(s) {
  ...
  // s.strokeWidth = s.strokeWidth * (s.scaleX-1)
  s.scaleX = 1;
  s.scaleY = 1;
}
function groupWidthHeightTransform(s) {
  // TODO Strokewidth parameter
  s.width = s.width; //* s.scaleX+ s.strokeWidth * (s.scaleX - 1);
  s.height = s.height; //* s.scaleY+ s.strokeWidth * (s.scaleY - 1);
  // s.scaleX = 1;
  // s.scaleY = 1;
  let stroke_width = 1 / s.scaleX;
  //fabric.log(stroke_width)
  for (const o of s._objects) {
    o.strokeWidth = stroke_width * 2;
  }
}
...
module.exports = function rescale2resize(canvas) {
  ...
  canvas.on("before:selection:cleared", function (opt) {
    ...
    // if the selection wasn't on a scaled group, then
    // this function is not needed --> return
    if (group.type !== "group" || group.scaleX === 1) {
      return;
    }
